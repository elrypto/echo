...
export default class LoomTest extends React.Component {
  ...
  async getValue() {
    try {
      ...
      console.log(`getValue JSON returns ${JSON.stringify(value)} and the actual val is:${value["0"]}`);
   
    } catch (err) {
      console.error('Ops, some error happened on get:', err)
    }
  }
    ...
    return (
      <div className="container" style={{ marginTop: 10 }}>
        <form onSubmit={e => { e.preventDefault(); }}>
          ...
          <button type="button" disabled={this.state.isSending} className="btn btn-primary" onClick={() => this.confirmValue()}>Confirm</button>
          <button type="button" className="btn btn-primary" onClick={() => this.getValue()}>--Get--</button>
        </form>
